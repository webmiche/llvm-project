...
; Function Attrs: minsize nofree nosync nounwind optsize memory(readwrite, inaccessiblemem: none) uwtable
...
entry:
  ...
  store i64 0, ptr %vli_pos_internal, align 8, !tbaa !5
...
if.end13:                                         ; preds = %if.end9, %if.then
  %5 = phi i64 [ 0, %if.then ], [ %3, %if.end9 ]
  %6 = phi i64 [ %0, %if.then ], [ %4, %if.end9 ]
  %vli_pos.addr.0 = phi ptr [ %vli_pos_internal, %if.then ], [ %vli_pos, %if.end9 ]
  br label %do.body

do.body:                                          ; preds = %do.cond, %if.end13
  %7 = phi i64 [ %5, %if.end13 ], [ %add, %do.cond ]
  %8 = phi i64 [ %6, %if.end13 ], [ %13, %do.cond ]
  %arrayidx = getelementptr inbounds i8, ptr %in, i64 %8
  %9 = load i8, ptr %arrayidx, align 1, !tbaa !9
  %inc = add nuw i64 %8, 1
  store i64 %inc, ptr %in_pos, align 8, !tbaa !5
  %10 = and i8 %9, 127
  %conv14 = zext i8 %10 to i64
  %11 = load i64, ptr %vli_pos.addr.0, align 8, !tbaa !5
  %mul15 = mul i64 %11, 7
  %shl = shl i64 %conv14, %mul15
  %add = add i64 %shl, %7
  store i64 %add, ptr %vli, align 8, !tbaa !5
  %12 = load i64, ptr %vli_pos.addr.0, align 8, !tbaa !5
  %inc16 = add i64 %12, 1
...
cleanup:                                          ; preds = %do.body
  ...
  br i1 %cmp32.not, label %cleanup41, label %do.cond

do.cond:                                          ; preds = %cleanup
  %13 = load i64, ptr %in_pos, align 8, !tbaa !5
  %cmp36 = icmp ult i64 %13, %in_size
...
cleanup41:                                        ; preds = %cleanup, %if.then21, %if.end28, %if.end9, %if.end5, %lor.lhs.false, %if.then, %do.end
  %retval.2 = phi i32 [ %cond40, %do.end ], [ 9, %if.then ], [ 11, %lor.lhs.false ], [ 11, %if.end5 ], [ 10, %if.end9 ], [ 9, %if.then21 ], [ %cond, %if.end28 ], [ 9, %cleanup ]
...
attributes #0 = { minsize nofree nosync nounwind optsize memory(readwrite, inaccessiblemem: none) uwtable "min-legal-vector-width"="0" "no-trapping-math"="true" "stack-protector-buffer-size"="8" "target-cpu"="x86-64" "target-features"="+cmov,+cx8,+fxsr,+mmx,+sse,+sse2,+x87" "tune-cpu"="generic" }
