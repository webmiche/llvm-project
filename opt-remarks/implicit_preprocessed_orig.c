...
void marc_arcs(network_t* net, long *new_arcs, long *new_arcs_array, arc_p **arcs_pointer_sorted)
{
  ...
  if (net->n_trips <= 15000)
    max_new_arcs = net->max_residual_new_m - 1000000;
  else
    max_new_arcs = net->max_residual_new_m - 4000000;
  *new_arcs = 0;
  ...
  *new_arcs = 0;
  ...
  if ((*new_arcs + net->m) % 4000 != 0)
      net->full_groups = net->nr_group - (4000 - ((*new_arcs + net->m) % 4000));
  else
...
long resize_prob( network_t *net )
{
    ...
    size_t off;
    ((void) sizeof ((net->max_new_m >= 3) ? 1 : 0), __extension__ ({ if (net->max_new_m >= 3) ; else __assert_fail ("net->max_new_m >= 3", "implicit.c", 214, __extension__ __PRETTY_FUNCTION__); }));
...
long price_out_impl( network_t *net )
{
  ...
  long size_del;
    ...
    if (net->n_trips <= 15000)
        num_switch_iterations = 1000;
    else
        num_switch_iterations = 2000;
    sorted_array = net->sorted_arcs;
    if (!sorted_array)
      return -1;
...
long suspend_impl( network_t *net, cost_t threshold, long all )
{
  ...
  while (net->full_groups < 0) {
    ...
    if( susp )
    {
        ...
        if ((net->m - susp) % 4000 != 0)
           net->full_groups = net->nr_group - (4000 - ((net->m - susp) % 4000));
        else
           net->full_groups = net->nr_group;
      while (net->full_groups < 0) {
        net->full_groups = net->nr_group + net->full_groups;
