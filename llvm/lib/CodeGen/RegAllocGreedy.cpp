...
#define DEBUG_TYPE "regalloc-greedy"

STATISTIC(NumGlobalSplits, "Number of split global live ranges");
STATISTIC(NumLocalSplits, "Number of split local live ranges");
STATISTIC(NumEvicted, "Number of interferences evicted");

static cl::opt<bool> RandomSpills("random-spills", cl::Hidden,
                                  cl::desc("Do some randome initial spills"),
                                  cl::init(false));

static cl::opt<bool> RandomPrio("random-prio", cl::Hidden,
                                cl::desc("Randomize priorities"),
                                cl::init(false));

static cl::opt<int> RandomSeed("random-seed", cl::Hidden,
                               cl::desc("Seed for random number generator"),
                               cl::init(0));
...
void RAGreedy::enqueue(PQueue &CurQueue, const LiveInterval *LI) {
  ...
  if (Stage == RS_New) {
    // get a random number
    int random = RandomSpills ? rand() % 1 : 1;
    if (random == 0) {
      Stage = RS_Spill;
    } else {
      Stage = RS_Assign;
...
unsigned RAGreedy::calculateRegionSplitCostAroundReg(MCPhysReg PhysReg,
                                                     AllocationOrder &Order,
                                                     BlockFrequency &BestCost,
                                                     unsigned &NumCands,
                                                     unsigned &BestCand) {
